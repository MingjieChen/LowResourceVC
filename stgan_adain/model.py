...
class ResidualBlock2D(nn.Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        self.conv_1 = nn.Conv2d(dim_in, dim_out, kernel_size=3, stride=1, padding=1, bias=False)
...
class ResidualBlockSplit(nn.Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        self.conv_1 = nn.Conv1d(dim_in, 2*dim_out, kernel_size=3, stride=1, padding=1, bias=False)
...
class SPEncoder(nn.Module):
    ...
    def __init__(self, num_speakers = 4, spk_cls = False):
        ...
        self.down_sample_1 = nn.Sequential(
            nn.Conv1d(in_channels=36, out_channels=256, kernel_size=5, stride = 2, padding=2, bias=False),
            nn.LeakyReLU(0.02),
        )
        self.down_sample_2 = nn.Sequential(
            nn.Conv1d(in_channels=256, out_channels=256, kernel_size=3, stride=2, padding=1, bias=False),
...
class GeneratorSplit(nn.Module):
    ...
    def __init__(self, num_speakers=4, aff = True):
        ...
        self.up_sample_1 = nn.ConvTranspose2d(256, 2*256, kernel_size = 4, stride = 2, padding = 1)
        self.up_in_1 = nn.InstanceNorm2d(2*256, affine = True)
        self.up_relu_1 = nn.GLU( dim = 1)
        #self.up_relu_1 = nn.LeakyReLU(0.2)

        self.up_sample_2 = nn.ConvTranspose2d(256, 2 * 128, kernel_size = 4, stride = 2, padding = 1 )
...
class Discriminator(nn.Module):
    ...
    def __init__(self, num_speakers=10):
        ...
        self.conv_layer_1 = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=256, kernel_size=(3, 3), stride=(1, 2), padding=1),
            nn.GLU(dim = 1)
        )
        ...
        self.down_sample_4 = nn.Sequential(
            nn.Conv2d(in_channels=1024, out_channels=1024, kernel_size=(1, 5), stride=(1, 2), padding=(0, 2), bias = False),
            nn.GLU(dim = 1)
        )
