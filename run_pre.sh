...
wav_path=/share/mini/data/audvis/pub/vc/studio/en/vctk/v1/wav48/
...
mc_path=$root/dump/0721mc_10spk_22050/
#script_path=/share/mini1/res/t/vc/studio/timap-en/vctk/stgan/preprocess.py

python_script=$root/vc-gan/preprocess_vctk.py


$PYTHON $python_script   --do_resample --do_split --sample_rate 22050 --origin_wavpath $wav_path  \
                         --target_wavpath $resample_path \
                         --mc_dir_train $mc_path/train --mc_dir_test $mc_path/test --speaker_used_path $mc_path/speaker_used.json \
                         --num_workers 20 --speaker_list  p262 p272 p229 p232 p292 p293 p360 p361 p248 p251
