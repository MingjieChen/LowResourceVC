...
class Generator(nn.Module):
    ...
    def __init__(self, device, num_speakers=4):
        ...
        '''
        self.residual_1 = ResidualBlock(dim_in=256, dim_out=256, style_num=num_speakers, device = device)
        self.residual_2 = ResidualBlock(dim_in=256, dim_out=256, style_num=num_speakers, device = device)
        self.residual_3 = ResidualBlock(dim_in=256, dim_out=256, style_num=num_speakers, device = device)
        self.residual_4 = ResidualBlock(dim_in=256, dim_out=256, style_num=num_speakers, device = device)
        self.residual_5 = ResidualBlock(dim_in=256, dim_out=256, style_num=num_speakers, device = device)
        self.residual_6 = ResidualBlock(dim_in=256, dim_out=256, style_num=num_speakers, device = device)
        self.residual_7 = ResidualBlock(dim_in=256, dim_out=256, style_num=num_speakers, device = device)
        self.residual_8 = ResidualBlock(dim_in=256, dim_out=256, style_num=num_speakers, device = device)
        self.residual_9 = ResidualBlock(dim_in=256, dim_out=256, style_num=num_speakers, device = device)
...
class Discriminator(nn.Module):
    ...
    def forward(self, x, c, c_):
        ...
        in_prod = torch.sum(p * h, dim=1, keepdim=True)
...
class PatchDiscriminator(nn.Module):
    ...
    def __init__(self, input_size=(36, 256), conv_dim=64, repeat_num=4, num_speakers=10):
    ...
    def forward(self, x, c_src, c_trg):
        ...
        h_mean = torch.sum(h, dim = [2,3])
        in_prod = h_mean * p
        h = h + in_prod.unsqueeze(2).unsqueeze(3)
